...
(defn new-str-builder
  [init]
  #?(:clj  (StringBuilder. init)
     :cljs (StringBuffer. init)))

(defn str-builder-length
  [s]
  #?(:clj  (.length s)
     :cljs (.getLength init)))
...
(defn- dot-attrs
  [attrs]
  (when (seq attrs)
    (let [sb (new-str-builder "[")]
      (doseq [[k v] attrs]
        (when (seq (str v))
          (when (< 1 (str-builder-length sb))
            (.append sb \,))
          (doto sb
            (.append \")
